"""
Configuration management for Config Service
"""
...
def get_safe_private_key(private_key: str) -> Optional[str]:
    ...
    logger.warning("Invalid or placeholder private key detected. SSI service will not be initialized.")
    return None
...
class Settings(BaseSettings):
    ...
    # API Configuration
    app_name: str = Field(default="Config Service", description="Application name")
    app_version: str = Field(default="1.0.0", description="Application version")
    app_description: str = Field(
        default="Centralized configuration management for the trading system",
    ...
    # Feature Flags
    enable_ssi_service: bool = Field(default=True, description="Enable SSI service")
    ...
    def validate_cors_origins(cls, v):
        ...
        if "*" in v and len(v) > 1:
            logger.warning("Wildcard origin '*' should be used alone")
        return v
    ...
    def get_ssi_config(self) -> dict:
        """Get SSI configuration"""
        return {
            "consumer_id": self.consumer_id,
            "consumer_secret": self.consumer_secret,
            "private_key": self.safe_private_key,
            "public_key": self.public_key,
            "fc_trading_url": self.fc_trading_url,
            "fc_data_url": self.fc_data_url,
            "fc_trading_stream_url": self.fc_trading_stream_url,
